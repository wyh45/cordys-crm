...
<script lang="ts" setup>
  ...
  import { initDevModeAuth, isDevMode } from '@/utils/devMode';
    ...
    formRef.value?.validate(async (errors) => {
      if (!errors) {
        ...
        try {
          ...
          if (isDevMode()) {
            initDevModeAuth();
            appStore.setOrgId('dev-org-id');
            userStore.setInfo({
              id: 'admin',
              name: userInfo.value.username || 'Developer',
              ...
              roles: ['admin'],
              userId: 'admin',
              userName: userInfo.value.username || 'Developer',
  ...
  onMounted(async () => {
    if (!props.isPreview) {
      ...
      // Dev mode: skip API calls and use mock data
      if (isDevMode()) {
        initDevModeAuth();
        preheat.value = false;
        return;
