export default {
    ...
    'system.business.authenticationSettings.togglePlatformTip': '注意：扫码登录及消息通知会调用切换后平台（{type}）的配置，可能导致登录失败或消息接收异常，建议您谨慎操作！',
    'system.business.authenticationSettings.changeCorpIdTip': '当前企业 ID 与系统存档不匹配！对接新企业会直接清空历史企业用户数据，清谨慎操作！',
    ...
    'system.config.page.iconTip': '顶部网站显示的 logo ；建议使用 SVG 或 PNG 格式透明背景图片，高度小于 32px；图片大小仅支持 200kb 以内',
    'system.config.page.loginLogo': '登录 Logo',
    'system.config.page.loginLogoTip': '登录页面右侧显示的 logo；建议使用 SVG 或 PNG 格式透明背景图片，高度小于 48px；图片大小仅支持 200kb 以内',
    'system.config.page.loginBg': '登录背景图',
    'system.config.page.loginBgTip': '背景图建议使用 SVG 格式；矢量图建议尺寸 1440*900，位图建议尺寸1920*1080；图片大小仅支持 800kb 以内',
    ...
    'system.config.page.platformLogoTip': '主题栏显示的 logo；建议使用 SVG 或 PNG 格式透明背景图片，高度小于 32px；图片大小仅支持 200kb 以内',
