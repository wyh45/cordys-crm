...
import { deleteClueFollowPlan, deleteClueFollowRecord, deleteCustomerFollowPlan, deleteCustomerFollowRecord, deleteOptFollowPlan, deleteOptFollowRecord, getClueFollowPlanList, getClueFollowRecordList, getCluePoolFollowRecordList, getCustomerFollowPlanList, getCustomerFollowRecordList, getCustomerOpenSeaFollowRecordList, getOptFollowPlanList, getOptFollowRecordList, getPersonalFollow, updateClueFollowPlanStatus, updateCustomerFollowPlanStatus, updateOptFollowPlanStatus, } from '@/api/modules';
...
export default function useFollowApi(followProps) {
    ...
    function transformField(item, optionMap) {
        const methodKey = type.value === 'followPlan' ? 'method' : 'followMethod';
        let followMethod;
        if (optionMap) {
            followMethod =
                optionMap[methodKey]?.find((e) => e.id === item[methodKey])?.name || '-';
        }
        return {
            ...item,
    ...
    async function loadFollowList(refresh = true) {
        try {
            ...
            if (res) {
                const newList = res.list.map((item) => transformField(item, res?.optionMap));
    ...
    // 更新计划状态
    async function changePlanStatus(item) {
        try {
...
//# sourceMappingURL=useFollowApi.js.map