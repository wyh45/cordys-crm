...
export const getFormConfigApiMap: Record<FormDesignKeyEnum, (id?: string) => Promise<FormDesignConfigDetailParams>> = {
  ...
  // 健康管理 — 无表单设计配置，使用空配置存根
  [FormDesignKeyEnum.HEALTH_ARCHIVE]: async () => ({ fields: [], formProp: {} } as any),
  [FormDesignKeyEnum.HEALTH_FOLLOW]: async () => ({ fields: [], formProp: {} } as any),
  [FormDesignKeyEnum.HEALTH_KNOWLEDGE]: async () => ({ fields: [], formProp: {} } as any),
};
...
export const createFormApi: Record<FormDesignKeyEnum, (data: any) => Promise<any>> = {
  ...
  // 健康管理
  [FormDesignKeyEnum.HEALTH_ARCHIVE]: async (data: any) => {
    const { addHealthArchive } = await import('@/api/modules');
    return addHealthArchive(data);
  },
  [FormDesignKeyEnum.HEALTH_FOLLOW]: async (data: any) => {
    const { saveHealthFollow } = await import('@/api/modules');
    return saveHealthFollow(data);
  },
  [FormDesignKeyEnum.HEALTH_KNOWLEDGE]: async (data: any) => {
    const { saveHealthKnowledge } = await import('@/api/modules');
    return saveHealthKnowledge(data);
  },
};
...
export const updateFormApi: Record<FormDesignKeyEnum, (data: any) => Promise<any>> = {
  ...
  // 健康管理
  [FormDesignKeyEnum.HEALTH_ARCHIVE]: async (data: any) => {
    const { updateHealthArchive } = await import('@/api/modules');
    return updateHealthArchive(data);
  },
  [FormDesignKeyEnum.HEALTH_FOLLOW]: async (data: any) => {
    const { saveHealthFollow } = await import('@/api/modules');
    return saveHealthFollow(data);
...
export const getFormListApiMap: Partial<Record<FormDesignKeyEnum, (data: any) => Promise<CommonList<any>>>> = {
  ...
  [FormDesignKeyEnum.HEALTH_ARCHIVE]: getHealthArchiveList,
  [FormDesignKeyEnum.HEALTH_FOLLOW]: getHealthFollowList,
  [FormDesignKeyEnum.HEALTH_KNOWLEDGE]: getHealthKnowledgeList,
};
