...
import { getGlobalCluePoolList, getGlobalCustomerContactList, getGlobalCustomerList, getGlobalOpenSeaCustomerList, getGlobalSearchClueList, globalSearchOptPage, } from '@/api/modules';
import useOpenNewPage from '@/hooks/useOpenNewPage';
import { useAppStore } from '@/store';
import { ClueRouteEnum, CustomerRouteEnum, OpportunityRouteEnum } from '@/enums/routeEnum';
export const getSearchListApiMap = {
...
// 固定展示字段
export const fixedFieldKeyListMap = {
...
export default async function useSearchTable(props) {
    ...
    function openNewPageOpportunity(row, isCustomer = false) {
    ...
    function openNewPageCustomerContact(row) {
    ...
    const systemFieldList = computed(() => [
        ...
        {
            dataIndex: 'poolName',
            title: props.searchTableKey.value === FormDesignKeyEnum.SEARCH_ADVANCED_PUBLIC
    ...
    const displayedColumnList = computed(() => {
        if (!props.searchTableKey.value)
            return [];
        // 转换成同属性后查重
        const selectedFieldKeyList = props.selectedFieldIdList.value.map((item) => props.fieldList.value.find((i) => i.id === item)?.dataIndex);
    ...
    const columns = computed(() => {
        const resultColumns = displayedColumnList.value.map((field) => {
            // 名称
            ...
            if (field.dataIndex === 'stage') {
                return {
                    ...
                    key: field.dataIndex,
                    render: (row) => {
                        const step = appStore.stageConfigList.find((e) => e.value === row.stage);
    ...
    const useTableRes = useTable((params) => api.value(params), {
        showPagination: true,
        ...
        const fieldMap = new Map(item.moduleFields?.map((field) => [field.fieldId, field.fieldValue]));
        const fieldAttr = Object.fromEntries(displayedColumnList.value
            .filter((i) => i.id && fieldMap.has(i.id))
            .map((i) => [i.dataIndex, fieldMap.get(i.id)]));
        return {
            ...item,
            ...fieldAttr,
        };
...
//# sourceMappingURL=useSearchTable.js.map