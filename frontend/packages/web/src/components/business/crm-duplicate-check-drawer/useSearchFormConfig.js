...
export default function useSearchFormConfig() {
    ...
    // 最终自定义字段对应的类型Map
    const searchFieldMap = ref({});
    const allFieldMap = ref({});
    async function initSearchFormConfig() {
        try {
            ...
            const promises = uniqueConfigs.map(async (value) => {
                const configKey = Object.keys(configMap).find((key) => configMap[key].includes(value));
                if (!configKey)
                    return;
                if (!hasAnyPermission(configPermissionMap[value]))
                    return;
                const res = await getFormConfigApiMap[configKey]();
                ...
                configMap[configKey]?.forEach((configValue) => {
...
//# sourceMappingURL=useSearchFormConfig.js.map