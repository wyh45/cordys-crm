 ...
 */
export function insertAtomic(container, atomicNode) {
 ...
 */
export function createFunctionNode(fnName, options) {
 ...
 * @returns 函数根节点
 */
export function createFunctionRootNode(fnName, argsFragment) {
...
function getFormulaNodeColor(token) {
    if (token.type === 'function') {
        return FUN_COLOR;
    }
    // —— 数组字段（二级路径）
    if (token.fieldId?.includes('.')) {
        return ARRAY_COLOR;
    }
    // —— 普通字段
...
export function createFieldNode(token) {
    ...
    if (token?.fieldType) {
        node.dataset.fieldType = token?.fieldType;
    }
 ...
 * 回显渲染 tokens
 */
export function renderTokens(tokens, startIndex = 0, stopAtRightParen = false) {
 ...
 */
export function renderTokensToEditor(editor, tokens) {
    editor.innerHTML = '';
