...
const { t } = useI18n();
export function serializeNode(node, fieldNameMap, fields) {
    let result;
    switch (node.type) {
        case 'literal': {
...
// 回显解析ast 收集保存入参
export function serializeFormulaFromAst(astList, fieldNameMap // fieldId -> 中文名
) {
 ...
 * @returns 公式的token列表
 */
export function tokenizeFromSource(source, fieldMap) {
    const tokens = [];
    let i = 0;
    while (i < source?.length) {
        ...
        // ---------- function ----------
            ...
            let j = i;
            while (j < source?.length && /[A-Z]/.test(source[j]))
                j++;
...
//# sourceMappingURL=serializeFormulaFromAst.js.map