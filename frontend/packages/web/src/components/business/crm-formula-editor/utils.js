...
export function createRangeFromStringIndex(root, start, end) {
...
export function clearErrorHighlight(editor) {
    editor.querySelectorAll('.formula-error').forEach((el) => {
        el.replaceWith(document.createTextNode(el.textContent || ''));
    });
}
export function applyDiagnosticsHighlight(editor, diagnostics) {
    clearErrorHighlight(editor);
    diagnostics.forEach((diag) => {
        if (!diag.highlight?.range)
            return;
...
export function safeParseFormula(formulaString) {
    try {
 ...
 */
export function ensureCaretAroundAtomic(node) {
 ...
 * @param root 编辑器根节点
 */
export function normalizeEditorStructure(root) {
 ...
 * @returns void
 */
export function insertRangeAtomic(root, atomic) {
 ...
 */
export function isAtomicNode(node) {
 ...
 * @returns void
 */
export function ensureTextAround(node) {
 ...
 * @returns void
 */
export function deleteAtomicNode(atomic) {
    const parent = atomic.parentNode;
    if (!parent)
        return;
    ensureTextAround(atomic);
    const leftText = atomic.previousSibling;
    atomic.remove();
    const range = document.createRange();
 ...
 */
export function findLeftAtomicDeep(container, offset) {
    let node = null;
    //  在 text node 中
    ...
    if (node?.nodeType === Node.TEXT_NODE && node.textContent === ')') {
        ...
        if (maybeArgs instanceof HTMLElement &&
 ...
 */
export function isRangeInsideSameFnArgs(range) {
 ...
 */
export function ensureFnArgsHasCaretText(argsEl) {
