...
const { t } = useI18n();
function isNumericLikeNode(node) {
    if (!node)
        return false;
    ...
    if (node.type === 'field') {
        return [FieldTypeEnum.INPUT_NUMBER, FieldTypeEnum.DATE_TIME, FieldTypeEnum.FORMULA].includes(node.fieldType);
    }
    if (node.type === 'binary') {
...
export default function diagnoseBinaryOperandType(node) {
    const diagnostics = [];
    if (node.type !== 'binary') {
        return diagnostics;
    }
...
//# sourceMappingURL=diagnoseBinaryOperandType.js.map