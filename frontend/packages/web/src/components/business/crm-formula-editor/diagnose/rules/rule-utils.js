...
export function isColumnField(fieldId) {
    return fieldId.includes('.');
}
export function isLogicalNode(node) {
    if (!node)
        return false;
    if (node.type === 'compare')
        return true;
    if (node.type === 'literal' && node.valueType === 'boolean')
        return true;
 ...
 */
export function isTextNumberDateNode(node) {
    if (!node)
        return false;
    ...
    if (node.type === 'field') {
        if (node.fieldType === FieldTypeEnum.DATE_TIME)
            return true;
        if (node.fieldType === FieldTypeEnum.INPUT_NUMBER)
            return true;
        // 其他字段按“文本”处理
        return true;
    }
...
export function isTextNumberDateOrLogicalNode(node) {
 ...
 */
export function isLogicalLikeNodeForAnd(node) {
    if (!node)
        return false;
    // 原生逻辑值
    ...
    // 数字字段 / 日期字段允许
    if (node.type === 'field') {
        return [FieldTypeEnum.INPUT_NUMBER, FieldTypeEnum.DATE_TIME].includes(node.fieldType);
    }
    // 返回数值 / 日期的函数允许
    if (node.type === 'function') {
