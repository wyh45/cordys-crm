export function createDiagnoseContext(tokens) {
    return {
        tokens,
        index: 0,
        diagnostics: [],
        ...
        push(diag) {
            ...
            if (!tokenRange || tokenRange.length !== 2)
                return;
...
export function isIllegalFunctionCall(prev, cur) {
    return prev?.type === 'paren' && prev.value === ')' && cur?.type === 'paren' && cur.value === '(';
}
// 是否在函数参数内
export function isInsideFunctionArgs(tokens, index) {
    let depth = 0;
