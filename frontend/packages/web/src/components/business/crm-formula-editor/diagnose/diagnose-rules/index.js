...
const duplicateOperatorRule = {
    ...
    check(ctx) {
        ...
        if (cur?.type !== 'operator' || prev?.type !== 'operator')
            return;
        // 只在连续 operator 的起点报
        if (index > 1 && tokens[index - 2]?.type === 'operator')
            return;
        let end = index;
...
const adjacentValueRule = {
    ...
    check(ctx) {
        ...
        if (isIllegalFunctionCall(prev, cur))
            return;
