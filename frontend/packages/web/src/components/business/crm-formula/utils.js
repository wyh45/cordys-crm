...
export function hydrateIRNumberType(node, fieldTypeMap) {
    ...
    switch (node.type) {
        ...
        case IRNodeType.Function: {
            node.args = node.args.map((arg) => hydrateIRNumberType(arg, fieldTypeMap));
            return node;
        }
...
export function getFormulaDataSourceDisplayValue(formulaDataSource, fieldId, rawValue) {
...
export function flatAllFields(fields, options) {
    const result = [];
    const resolveFieldId = (e, inSubTable) => {
...
export function normalizeFormulaNumber(n) {
 ...
 * 保留小数位，不进行四舍五入
 */
export function keepDecimal(value, digits = 2) {
