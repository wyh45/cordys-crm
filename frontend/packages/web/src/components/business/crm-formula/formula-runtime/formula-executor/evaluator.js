...
function dateToSerial(date) {
    ...
    if (date instanceof Date) {
        t = new Date(date.getFullYear(), date.getMonth(), date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds()).getTime();
    }
    else {
...
export function resolveFieldValue(rawVal, node, ctx) {
...
export default function evaluateIR(node, ctx) {
    switch (node.type) {
        ...
        default:
            ctx.warn?.(`Unknown node type ${node.type}`);
            return null;
    }
}
