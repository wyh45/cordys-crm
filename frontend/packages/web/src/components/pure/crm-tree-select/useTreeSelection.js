export default function useTreeSelection(selectedModuleProps) {
    const { modulesTree, moduleCount } = selectedModuleProps;
    const moduleTree = ref(modulesTree);
    const modulesCount = ref(moduleCount);
    // 模块全选keys
    const checkedKeys = ref(Array.isArray(selectedModuleProps.value) ? selectedModuleProps.value : [selectedModuleProps.value]);
    // 模块半选keys
    const halfCheckedKeys = ref([]);
    // 模块配置集合
    const selectedModulesMaps = ref({});
    // 是否全选
    const isCheckedAll = ref(false);
    // 是否半选
    const indeterminate = ref(false);
    // 设置节点
    function setNode(nodeData, checked) {
    ...
    // 选中当前节点 && 取消当前节点
    function selectParent(nodeData, isSelected) {
        setNode(nodeData, !isSelected);
    }
    // 初始化左侧模块节点选中当前以及子节点
    function processAllCurrentNode(node, check) {
        if (node?.children && node?.children?.length) {
            node.children.forEach((childrenNode) => processAllCurrentNode(childrenNode, check));
        }
        setNode(node, check);
    }
    // 选中当前节点以及子节点
    function checkNode(_checkedKeys, checkedData) {
        const { checked, node } = checkedData;
        processAllCurrentNode(node, checked);
    }
    // 全选全部&取消全选
    function setSelectAll(tree, checkedAll) {
        tree.forEach((node) => {
    ...
    // 选中全部所有的模块
    function checkAllModule(value, ev) {
    ...
    watch(() => selectedModulesMaps.value, (val) => {
    ...
    watch(() => selectedModuleProps.modulesTree, (val) => {
        setModuleTree(val);
    }, {
        immediate: true,
        deep: true,
    });
    watch(() => selectedModuleProps.moduleCount, (val) => {
        setModuleCount(val);
    }, {
        immediate: true,
        deep: true,
...
//# sourceMappingURL=useTreeSelection.js.map