...
export default function useChart(props) {
    ...
    const xyAxis = computed(() => ({
        ...
        dataZoom: props.xData?.value && props.xData?.value.length >= 30
            ? [
                {
                    type: 'inside',
                    realtime: true,
    ...
    const pieConfig = computed(() => ({
        ...
        legend: {
            ...
            height: '80%',
            formatter(name) {
                const item = props.data.value.find((e) => e.name === name);
                return item
                    ? `{name|${name}}  {value|${abbreviateNumber(item.value, '').value}${abbreviateNumber(item.value, '').unit}} {percent|${percentMap.value[item.name]}%}`
                    : name;
            },
            textStyle: {
                width: 400,
    ...
    const chartOptions = ref(deepMerge({
        title: {
            text: title.value,
            textAlign: 'left',
            left: '1%',
