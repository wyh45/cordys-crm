    ...
    <template v-if="!props.notShowTable">
      <div class="relative min-h-0 flex-1">
        <n-data-table
          ...
          :row-key="getRowKey"
          flex-height
          :class="`absolute inset-0 ${props.notShowTableFilter ? 'not-show-filter' : ''} ${
    ...
    <template v-if="!attrs?.hiddenTotal || (attrs?.hiddenTotal && isFullScreen) || hasFinished">
      ...
      <div v-else class="crm-table-bottom-tip flex text-center">
        <div :class="`flex flex-1 items-start`">
          <div v-if="(!attrs?.hiddenTotal || (attrs?.hiddenTotal && isFullScreen)) && !attrs?.customTotal && (attrs?.crmPagination as PaginationProps)?.itemCount">
...
<script lang="ts" setup>
  ...
  const props = defineProps<{
    ...
    paginationType?: 'pagePagination' | 'scrollPagination';
  }>();
  ...
  async function initPaginationType(load: boolean = false) {
    if (props.paginationType) {
      paginationType.value = props.paginationType;
      return;
    }
    if (attrs.showSetting) {
