...
// 判断当前一级菜单是否有权限
export function topLevelMenuHasPermission(route) {
    const userStore = useUserStore();
    const appStore = useAppStore();
    const { moduleConfigList } = appStore;
    if (moduleConfigList.length &&
        !moduleConfigList.filter((e) => e.enable).some((e) => e.moduleKey === route.name)) {
        // 没有配置的菜单不显示
        return false;
    }
    if (userStore.isAdmin) {
...
export function getFirstRouteNameByPermission(routerList) {
...
// 判断当前路由名有没有权限
export function routerNameHasPermission(routerName, routerList) {
...
// 找到当前路由下 第一个由权限的子路由
export function getFirstRouterNameByCurrentRoute(parentName) {
