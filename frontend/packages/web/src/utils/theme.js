 ...
 * @returns 颜色值
 */
export function getRGBinnerVal(color) {
    return color
        .rgb()
        .toString()
        .replace(/rgba?\(|\)/g, '');
 ...
 */
export function setCustomTheme(primaryColor) {
    ...
    const transferColor = (primaryColorString) => {
 ...
 * @param color 平台色
 */
export function setPlatformColor(color, isFollow = false) {
    const styleTag = document.createElement('style');
    const appStore = useAppStore();
 ...
 */
export function watchStyle(val, pageConfig) {
    if (val === 'default') {
        // 默认就是系统自带的颜色
        resetStyle();
    }
 ...
 * @param pageConfig 页面配置对象
 */
export function watchTheme(val, pageConfig) {
    if (val === 'default') {
        resetTheme();
