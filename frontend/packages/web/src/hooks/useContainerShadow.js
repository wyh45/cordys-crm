...
/**
 * 监听指定容器的滚动事件，以给容器添加顶部和底部的阴影，需要给指定的容器添加 .crm-container--shadow-y() 样式类使用阴影
 * @param options ContainerShadowOptions
 */
export default function useContainerShadow(options) {
    const isArrivedTop = ref(true);
    const isArrivedBottom = ref(true);
    const isInitListener = ref(false);
    const containerRef = ref(options.content);
    function setContainer(dom) {
        if (dom) {
            containerRef.value = dom;
        }
    }
    function calculateArrivedPosition(listContent) {
...
//# sourceMappingURL=useContainerShadow.js.map