...
function createApi(coreApi, opt) {
    return new Proxy(coreApi, {
        get(target, propKey) {
            if (typeMap.includes(propKey)) {
...
// 创建 Dialog ，合并额外配置
function createDialog(dialog, opt) {
    // 给对话框的按钮配置提供默认值类型和尺寸
...
function createNotification(notification, defaultOptions = {} // 限制通知最大弹出数量
) {
    const queue = [];
    ...
    return new Proxy(notification, {
        get(target, key) {
            ...
            if (typeof originalFn === 'function' && typeMap.includes(key)) {
                return (options = {}) => {
...
export default function useDiscreteApi(props) {
    if (!isInitialization) {
        ...
        const { message, notification, dialog, loadingBar } = createDiscreteApi(['message', 'dialog', 'notification', 'loadingBar'], props);
        // 使用 i18n 钩子获取国际化文本
        const { t } = useI18n();
        try {
...
//# sourceMappingURL=useDiscreteApi.js.map