...
import { getAccountContract, getAccountContractStatistic, getAccountPayment, getAccountPaymentRecord, getAccountPaymentRecordStatistic, getAccountPaymentStatistic, getCustomerInvoiceList, getCustomerInvoiceStatistic, } from '@/api/modules';
export default function useContractTimeline(formKey, sourceId) {
    const { t } = useI18n();
    const statisticListMap = {
    ...
    async function getFormConfig() {
        if (![FormDesignKeyEnum.CONTRACT_PAYMENT_RECORD, FormDesignKeyEnum.CONTRACT_PAYMENT].includes(formKey))
            return;
        try {
    ...
    function getDescription(item) {
        return descriptionListMap[formKey].map((desc) => {
            let itemValue = item[desc.key];
            if (itemValue === undefined && item.moduleFields?.length) {
                itemValue = item.moduleFields.find((field) => field.fieldId === desc.key)?.fieldValue;
            }
            ...
            if (formConfigField && formConfigField.type === FieldTypeEnum.DATE_TIME) {
                if (formConfigField.dateType === 'month') {
                    itemValue = dayjs(itemValue).format('YYYY-MM');
                }
                else if (formConfigField.dateType === 'date') {
                    itemValue = dayjs(itemValue).format('YYYY-MM-DD');
                }
                else {
                    itemValue = dayjs(itemValue).format('YYYY-MM-DD HH:mm:ss');
                }
            }
            return {
                ...desc,
    ...
    async function getStatistic() {
        try {
            ...
            statisticInfo.value = Object.entries(res).map((item) => {
                return {
                    key: item[0],
                    label: statisticListMap[formKey].find((i) => i.key === item[0])?.label,
                    value: item[1],
                };
            });
        }
...
//# sourceMappingURL=useContractTimeline.js.map