...
export default function useUser() {
    const { t } = useI18n();
    const logout = async (logoutTo, noRedirect, silence = false) => {
        try {
    ...
    const goUserHasPermissionPage = () => {
        ...
        const redirectHasPermission = redirect &&
            !redirect.includes(NO_RESOURCE_ROUTE_NAME) &&
            routerNameHasPermission(redirect, router.getRoutes());
        router.push({
            name: redirectHasPermission ? redirect : currentRouteName,
            query: {
                ...othersQuery,
            },
        });
