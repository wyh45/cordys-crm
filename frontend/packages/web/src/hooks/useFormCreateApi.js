...
import { FieldDataSourceTypeEnum, FieldRuleEnum, FieldTypeEnum, FormDesignKeyEnum, } from '@lib/shared/enums/formDesignEnum';
import { useI18n } from '@lib/shared/hooks/useI18n';
import { getCityPath, getIndustryPath, safeFractionConvert } from '@lib/shared/method';
import { dataSourceTypes, departmentTypes, formatNumberValueToString, getNormalFieldValue, getRuleType, initFieldValue, linkAllAcceptTypes, memberTypes, multipleTypes, parseFormDetailValue, parseModuleFieldValue, singleTypes, } from '@lib/shared/method/formCreate';
import { createFormApi, getFormConfigApiMap, getFormDetailApiMap, rules, updateFormApi, } from '@/components/business/crm-form-create/config';
import { checkRepeat } from '@/api/modules';
import useUserStore from '@/store/modules/user';
export default function useFormCreateApi(props) {
    const { t } = useI18n();
    const Message = useMessage();
    const userStore = useUserStore();
    const sourceName = ref(props.initialSourceName?.value); // 资源名称
    const collaborationType = ref(); // 协作类型-客户独有
    const specialInitialOptions = ref([]); // 特殊字段的初始化选项列表
    const descriptions = ref([]); // 表单详情描述列表
    const fieldList = ref([]); // 表单字段列表
    const fieldShowControlMap = ref({}); // 表单字段显示控制映射
    ...
    const originFormDetail = ref({});
    const moduleFormConfig = ref();
    // 详情
    const detail = ref({});
    const linkFormFieldMap = ref({}); // 关联表单字段信息映射
    ...
    function initFormShowControl(value) {
        // 读取整个显隐控制映射
     ...
     * @param form 表单数据
     */
    function formDescriptionShowControlRulesSet(form) {
        // 读取整个显隐控制映射
        Object.keys(fieldShowControlMap.value).forEach((fieldId) => {
            // 取出当前字段的所有规则
            ...
            if (field) {
                ...
                for (let i = 0; i < fieldRuleIds.length; i++) {
                    ...
                    if (controlField?.businessKey) {
                        ...
                        const formField = form.moduleFields?.find((moduleField) => moduleField.fieldId === controlField?.id);
    ...
    function formatInternalFieldValue(key, value) {
    ...
    function makeDescriptionItem(item, form) {
        if (item.show === false || !item.readable)
            return;
        ...
        else if (item.type === FieldTypeEnum.DATA_SOURCE &&
        ...
        else if (props.isContractTableDetail &&
            [FieldTypeEnum.DATA_SOURCE].includes(item.type) &&
            props.formKey.value === FormDesignKeyEnum.CONTRACT_SNAPSHOT &&
            [FieldDataSourceTypeEnum.BUSINESS, FieldDataSourceTypeEnum.QUOTATION].includes(item.dataSourceType) &&
            !item.resourceFieldId) {
            descriptions.value.push({
                label: item.name,
                value: parseFormDetailValue(item, form),
        ...
        try {
            ...
            fieldList.value.forEach((item) => {
                ...
                if ([FieldTypeEnum.SUB_PRICE, FieldTypeEnum.SUB_PRODUCT].includes(item.type) && item.subFields?.length) {
                    if (item.show === false || !item.readable || !value || value?.length === 0)
                        return;
                    descriptions.value.push({
                        label: item.name,
                        value,
    ...
    function makeLinkFormFields(field) {
        if (!formDetail.value[field.id])
            return;
        switch (true) {
            ...
            case multipleTypes.includes(field.type):
                ...
                if (field.type === FieldTypeEnum.INPUT_MULTIPLE) {
                    ...
                    linkFormFieldMap.value[field.id] = {
                        ...field,
                        value: formDetail.value[field.id].map((id) => field.options?.find((e) => e.value === id)?.label),
                    };
                }
                break;
    ...
    function fillLinkFormFieldValue(field, scenario) {
                ...
                if (linkField) {
                    switch (true) {
                        case dataSourceTypes.includes(field.type):
                            ...
                            formDetail.value[field.id] = linkField.value.map((e) => e.id);
                            break;
                        case multipleTypes.includes(field.type):
                            // 多选填充
                            if (field.type === FieldTypeEnum.INPUT_MULTIPLE) {
                        ...
                        case linkAllAcceptTypes.includes(field.type):
                            ...
                            if (dataSourceTypes.includes(linkField.type)) {
                                ...
                                // 联动的字段是省市区则填充城市路径
                                const addressArr = linkField.value.split('-') || [];
                                ...
                                formDetail.value[field.id] = linkField.value ? getIndustryPath(linkField.value) : '-';
                            }
    ...
    function transformFormDetailValue(item, res) {
        if (item.resourceFieldId) {
            ...
            // 其他的字段读取moduleFields
            const field = res.moduleFields?.find((moduleField) => moduleField.fieldId === item.id);
            if (field) {
    ...
    function makeSubFieldInitialOptions(subField, parentFieldId, res) {
        if (subField.businessKey) {
            ...
            if ([FieldTypeEnum.DATA_SOURCE].includes(subField.type)) {
                ...
                subField.initialOptions = options
                    ?.filter((e) => formDetail.value[parentFieldId]?.some((item) => item[subField.businessKey]?.includes(e.id)))
                    .map((e) => ({
                    ...e,
                    name: e.name || t('common.optionNotExist'),
                }));
            ...
            if ([FieldTypeEnum.DATA_SOURCE].includes(subField.type)) {
                // 处理成员和数据源类型的字段
                subField.initialOptions = options
                    ?.filter((e) => formDetail.value[parentFieldId]?.some((item) => item[subField.id]?.includes(e.id)))
                    .map((e) => ({
                    ...e,
                    name: e.name || t('common.optionNotExist'),
                }));
            ...
            fieldList.value.forEach((item) => {
                if ([FieldTypeEnum.SUB_PRICE, FieldTypeEnum.SUB_PRODUCT].includes(item.type) && item.subFields?.length) {
                    ...
                    item.subFields.forEach((subField) => {
                        ...
                        formDetail.value[item.id]?.forEach((subItem) => {
                            ...
                            if (subField.resourceFieldId) {
                                subItem[subField.id] = parseModuleFieldValue(subField, subItem[subField.id], res.optionMap?.[subField.id]);
                            }
                            else {
                                subItem[subField.businessKey || subField.id] = initFieldValue(subField, subItem[subField.businessKey || subField.id]);
                            }
                        });
                    });
                    return;
     ...
     */
    function specialFormFieldInit(field) {
        ...
        if ([FormDesignKeyEnum.CONTRACT_PAYMENT, FormDesignKeyEnum.CONTRACT_PAYMENT_RECORD, FormDesignKeyEnum.ORDER].includes(props.formKey.value) &&
        ...
        if ([FormDesignKeyEnum.FOLLOW_PLAN_CUSTOMER, FormDesignKeyEnum.FOLLOW_RECORD_CUSTOMER].includes(props.formKey.value) &&
            props.sourceId?.value) {
        ...
        if ([FormDesignKeyEnum.FOLLOW_PLAN_BUSINESS, FormDesignKeyEnum.FOLLOW_RECORD_BUSINESS].includes(props.formKey.value) &&
            props.sourceId?.value) {
    ...
    function initFormFieldConfig(fields) {
    ...
    function replaceRule(item, parentFieldId) {
        const fullRules = [];
        (item.rules || []).forEach((rule) => {
            ...
            if (staticRule) {
                ...
                if (staticRule.key === FieldRuleEnum.UNIQUE) {
                    if (parentFieldId) {
                        staticRule.validator = async (_rule, value) => {
                            if (!value || !value.length) {
                                return Promise.resolve();
                            }
                            const subFieldValues = formDetail.value[parentFieldId]?.map((subItem) => subItem[item.id]);
                            const valueCount = subFieldValues.filter((v) => v === value).length;
                            if (valueCount > 1) {
                                return Promise.reject(new Error(item.name.length
                        ...
                        staticRule.validator = async (_rule, value) => {
                            ...
                            try {
                                ...
                                if (info.repeat) {
                                    return Promise.reject(new Error(info.name.length
                    ...
                    staticRule.message = t(staticRule.message, { value: t(item.name) });
    ...
    function initForm(linkScenario) {
        fieldList.value.forEach((item) => {
            ...
            if (item.resourceFieldId && item.defaultValue) {
                defaultValue = parseModuleFieldValue(item, item.defaultValue, item.initialOptions || item.options?.map((opt) => ({ id: opt.value, name: opt.label })));
            ...
            else if ([FieldTypeEnum.DEPARTMENT, FieldTypeEnum.DEPARTMENT_MULTIPLE].includes(item.type) &&
    ...
    async function saveForm(form, isContinue, callback, noReset = false) {
        try {
            loading.value = true;
            const params = {
            ...
            fieldList.value.forEach((item) => {
                ...
                if (item.subFields?.length) {
                    ...
                    if (parentFieldDetail) {
                        parentFieldDetail.forEach((subItem) => {
...
//# sourceMappingURL=useFormCreateApi.js.map