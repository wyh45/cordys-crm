...
export default function useTableStore() {
    ...
    async function getTableColumnsMap(tableKey) {
        const tableColumnsMap = await getItem(tableKey);
        return tableColumnsMap;
    }
    async function setTableColumnsMap(tableKey, tableColumnsMap) {
        await setItem(tableKey, tableColumnsMap);
    }
    function columnsTransform(columns) {
    ...
    function sortByOldOrder(oldArr, newArr) {
        ...
        return [dragColumn, ...sorted, ...extra, operationColumn].filter(Boolean);
    }
    async function initColumn(tableKey, column) {
        try {
    ...
    // 表头显示设置的列
    async function getCanSetColumns(tableKey) {
        const tableColumnsMap = await getTableColumnsMap(tableKey);
        if (tableColumnsMap) {
            return tableColumnsMap.column.filter((item) => item.type !== SpecialColumnEnum.SELECTION &&
    ...
    // 在表格上展示的列
    async function getShowInTableColumns(tableKey) {
        const tableColumnsMap = await getTableColumnsMap(tableKey);
        if (tableColumnsMap) {
    ...
    async function setColumns(tableKey, columns) {
        try {
            const tableColumnsMap = await getTableColumnsMap(tableKey);
            if (tableColumnsMap) {
    ...
    async function getPageSize(tableKey) {
        const tableColumnsMap = await getTableColumnsMap(tableKey);
        return tableColumnsMap ? tableColumnsMap.pageSize : appStore.pageSize;
    }
    async function setPageSize(tableKey, pageSize) {
        try {
            const tableColumnsMap = await getTableColumnsMap(tableKey);
            if (tableColumnsMap) {
    ...
    async function setTableLineHeight(tableKey, layout) {
        try {
            const tableColumnsMap = await getTableColumnsMap(tableKey);
            if (tableColumnsMap) {
    ...
    async function getTableLineHeight(tableKey) {
        const tableColumnsMap = await getTableColumnsMap(tableKey);
        return tableColumnsMap?.layout || 'compact';
    }
    async function setTablePaginationType(tableKey, paginationType) {
        try {
            const tableColumnsMap = await getTableColumnsMap(tableKey);
            if (tableColumnsMap) {
    ...
    async function getTablePaginationType(tableKey) {
        const tableColumnsMap = await getTableColumnsMap(tableKey);
...
//# sourceMappingURL=useTableStore.js.map