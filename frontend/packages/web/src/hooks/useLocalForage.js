...
export default function useLocalForage() {
    ...
    const deserializeFunction = (funcStr) => {
        try {
    ...
    const deserializeValue = (val) => {
        if (typeof val === 'string' && val.startsWith('function:')) {
            return deserializeFunction(val.slice(9));
        }
        if (val && typeof val === 'object' && !Array.isArray(val)) {
            const newVal = { ...val };
            Object.keys(newVal).forEach((key) => {
     ...
     */
    const getItem = async (key, notIsolatedByProject = false) => {
        const itemKey = notIsolatedByProject ? key : `-${key}`;
        try {
            const res = await localforage.getItem(itemKey);
            if (!res) {
                return null;
            }
            return deserializeValue(res);
        }
        catch (e) {
            // eslint-disable-next-line no-console
            console.log(e);
     ...
     */
    const setItem = async (key, val, notIsolatedByProject = false) => {
        try {
            const itemKey = notIsolatedByProject ? key : `-${key}`;
...
//# sourceMappingURL=useLocalForage.js.map