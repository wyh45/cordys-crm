...
import { closeMessageSubscribe, getHomeMessageList, getKey, getModuleNavConfigList, getModuleTopNavList, getOpportunityStageConfig, getPageConfig, getSystemVersion, getThirdConfigByType, getThirdPartyResource, getUnReadAnnouncement, } from '@/api/modules';
...
const useAppStore = defineStore('app', {
    ...
    actions: {
         ...
         * 连接SSE消息订阅流
         */
        async connectSystemMessageSSE(callback) {
        ...
        setRestoreMenuTimeStamp(timeStamp) {
        ...
        async showSQLBot() {
            ...
            if (res.config.sqlBotChatEnable) {
                await loadScript(res.config?.appSecret, { identifier: CompanyTypeEnum.SQLBot });
            }
        },
        // 初始化页面配置
        async initPageConfig() {
            try {
                ...
                if (Array.isArray(res) && res.length > 0) {
                    ...
                    res.forEach((e) => {
                        const key = e.paramKey.split('ui.')[1]; // 参数名前缀ui.去掉
                        if (['icon', 'loginLogo', 'loginImage', 'logoPlatform'].includes(key)) {
                            ...
                            this.pageConfig[key] = e.paramValue;
                        }
                    });
